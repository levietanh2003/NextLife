...
class formatPrice {
    ...
    open fun formatPriceWaterFromString(input: String?): String {
        ...
        return "$formattedValue đ/m³"
    }

    open fun formatPriceElectricFromString(input: String?): String {
        // Kiểm tra nếu đầu vào là null hoặc rỗng
        if (input.isNullOrEmpty()) {
            return "0 VND"
        }
        ...
        return "$formattedValue đ/Kwh"
    }

    open fun formatPriceAdditionalFromString(input: String?): String {
        // Kiểm tra nếu đầu vào là null hoặc rỗng
        if (input.isNullOrEmpty()) {
            return "0 VND"
        }
